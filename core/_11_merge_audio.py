...
_NP_SCALAR_WRAPPER_RE = re.compile(r"np\.(?:float16|float32|float64|int16|int32|int64|uint16|uint32|uint64)\(([^()]+)\)")


def _parse_serialized_value(value):
    """Parse serialized list/tuple values saved in Excel cells."""
    if not isinstance(value, str):
        return value

    text = value.strip()
    # Normalize numpy scalar wrappers sometimes written into sheet text,
    # e.g. np.float64(1.23) -> 1.23
    prev = None
    while prev != text:
        prev = text
        text = _NP_SCALAR_WRAPPER_RE.sub(r"\1", text)

    try:
        return ast.literal_eval(text)
    except Exception as exc:
        raise ValueError(f"Failed to parse serialized value: {text[:120]}") from exc

def load_and_flatten_data(excel_file):
    """Load and flatten Excel data"""
    df = pd.read_excel(excel_file)
    lines = [_parse_serialized_value(line) for line in df['lines'].tolist()]
    lines = [item for sublist in lines for item in sublist]
    
    new_sub_times = [_parse_serialized_value(time) for time in df['new_sub_times'].tolist()]
    new_sub_times = [item for sublist in new_sub_times for item in sublist]
    new_sub_times = [[float(pair[0]), float(pair[1])] for pair in new_sub_times]
...
def get_audio_files(df):
    ...
    for index, row in df.iterrows():
        number = row['number']
        line_values = _parse_serialized_value(row['lines'])
